# TTRS ver 0.3.1
# Note: some code has been commented out, but not deleted as I might need it in the future.
...
class Gui4App: #The GUI was made with pygubu-designer
    ...
    def playAudio(self): #When button pressed, the audio plays within app
        self.setMessage(msg="Something went wrong ...") #If attempt fails, display this message
        
        
        #BUG: Plays old sound file even if accent has changed. After generating new sound, will still play the old file.
        #     Same problem for text changes. Seems that the issue is related to fileName, which stays the same
        
        #Idea 1: Add accent to play file. If the fileName accent != current stored variable accent, remake file.
        #        A noteable issue with this idea is that it won't account for text content changes (and would be
        #        a lil bit more complicated to implement) :/
        
        #Idea 2: Scrap the whole pre-making audio files process. This option will reduce complexity and kill the bug,
        #        but will make the playAudio function much much slower as it will need to create and destroy a new
        #        file every time it is called.
        
        #Idea 3: Only initialize the temp variables at the end of this function. This one seems eay to attempt, so I'll
        #        go ahead and try it out. Ngl, this idea is mostly a shot in the dark, but I've got a hunch that it might
        #        do the trick. Worst case scenario, it doesn't work; best case scenario, it works mysteriously and I'll
        #        have to figure out why later.
        ...
        if self.settingChanged == False and os.path.exists(self.tempPath):
            playsound(self.tempPath) #Play the audio
            self.setMessage(msg='Played audio') #Display success message
            
        else:
            self.setMessage(msg='Please generate audio first')
           
        #if os.path.exists(self.tempPath) == False: //Troubleshooting messages. Please ignore
            #print('Pathway does not exist')
        
        #if self.settingChanged == True:
            #print('Settings are still understood to have been changed')
    ...
    def makeAudio(self): #This will compile the info, send it to makeIt() if it can, and create a temp file for playback
        ...
        if self.accent != 'NULL' and self.fileTyp != 'NULL' and self.fileName != '' and self.content != '': #Every variable has inputs
