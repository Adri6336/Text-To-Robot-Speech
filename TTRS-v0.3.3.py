# TTRS ver 0.3.3 : Finally Added Word Wrap
...
class Gui4App:  # The GUI was made with pygubu-designer
    ...
    def __init__(self, master=None):
        ...
        self.txtEnter.config(width='60', wrap='word')
        _text_ = '''Enter Text Here!'''
        self.txtEnter.insert('0.0', _text_)
        self.txtEnter.place(bordermode='outside', relheight='0.42', relwidth='0.87', relx='0.07', rely='0.53', x='0',
                            y='0')
    ...
    def cleanUp(self):  # Clears the temporary audio files made when playing from app
        ...
        if os.path.exists('robotVoice\\log.TTRS'):  # If there's a log
            ...
            del delList[
                size - 1]  # The last element gets filled with a single comma, so lets remove that from the deletion list
            ...
            file = open('robotVoice\\log.TTRS',
                        'w')  # Opens the file to wipe it. By using the 'w', we're telling python to
            # prep the file for being written in
            file.write(
                '')  # Writing anything will tell Python to overrite the file. So to wipe the file, I'll just write an empty string to it
    ...
    def makeIt(self, fileName, text, accent,
               fileType):  # This is called to send the info to Google and have them generate a sound file
    ...
    def makeAudio(self):  # This will send the data to Google and produce a file
        ...
        if (self.accent != 'NULL' and self.fileTyp != 'NULL'
            ...
            self.makeIt(fileName=self.fileName, text=self.content, accent=self.accent,
                        fileType=self.fileTyp)  # send info to makeit fn
...
if __name__ == '__main__':
    ...
    root.wm_title("TTRS v0.3.3")  # Sets the title of the window to the string included as an argument
